...
<?php
  $selfJobIds = "SELECT JOBID FROM tbljobregistration WHERE APPLICANTID = ". $_SESSION['APPLICANTID'];
  $result2 = $conn->query($selfJobIds);

  // var_dump($_SESSION['APPLICANTID']);
  // var_dump($result2->fetch_assoc());
  // die();

  if ($result2->num_rows > 0){
    $jobIds = [];
    while($jobId = mysqli_fetch_array($result2))
    {
      if (!in_array($jobId["JOBID"], $jobIds)){
        $jobIds[] = $jobId["JOBID"];
      }
    }

    $categories = "SELECT CATEGORY FROM tbljob WHERE JOBID IN (". implode(',', $jobIds) . ");";
    $result3 = $conn->query($categories);
    ...
    $applicant_id = $_SESSION['APPLICANTID'];
    // $sql = "SELECT tbljob.* FROM tbljob LEFT JOIN views on tbljob.JOBID = views.job_id WHERE tbljob.CATEGORY in (\"". implode('","', $cats) ."\") AND tbljob.JOBID NOT IN (". implode(',', $jobIds) .") GROUP BY views.job_id ORDER BY SUM(views.visit_count) DESC LIMIT 10";
    $sql = "SELECT tbljob.* FROM tbljob LEFT JOIN views on tbljob.JOBID = views.job_id WHERE tbljob.CATEGORY in (\"". implode('","', $cats) ."\") AND tbljob.JOBID NOT IN (". implode(',', $jobIds) .") ORDER BY views.visit_count DESC LIMIT 10";
        
    
    $finalResults = $conn->query($sql);
    
    // var_dump($finalResults->fetch_assoc());
    // die();
  }
...
<div class="recommendations">
  ...
  <?php if (!empty($finalResults)): ?>
    ...
    <?php if ($finalResults->num_rows > 0): ?>
    <?php while ($row = $finalResults->fetch_assoc()): ?>
    
      <div class="recommendation" style="margin-bottom: 70px;">
      
        <h3>
          <?php 
            // Get job title

            $sql = "SELECT * FROM tbljob WHERE JOBID = " . $row["JOBID"];
        ...
        <a class="btn" href="index.php?q=viewjob&search=<?php echo $row["JOBID"]; ?>">
          View Job
        </a>
